class Main inherits IO{
    ...
    main():Object {
        let converter <- new A2I in 
        while looping loop {
            out_string("Please enter your command: ");
            somestr <- in_string();
            if somestr = "help" then
            {
                out_string("Available commands: load, print, merge i j, filterBy index {ProductFilter,RankFilter,SamePriceFilter}
                ,sortBy index {PriceComparator,RankComparator,AlphabeticComparator} {ascendent,descendent}")
            }
            else if somestr = "load" then
            {
                -- o sa ia si end line trebuie facut tokenize ??
            }
